...
from Single_view_3D_Loss import DINOv3PoseEstimator, Meca500Kinematics, IMAGE_RESOLUTION
...
JSON_DIR = "/home/najo/NAS/DIP/2025_ICRA_Multi_View_Robot_Pose_Estimation/dataset/Converted_dataset/Meca500_to_DREAM"
...
def visualize_3d_prediction(json_path, model, device='cuda'):
    """Visualize 3D model prediction on a single Meca500 sample."""
    ...
    pred_angles_np = pred_angles[0].cpu().numpy()[:6]  # Meca500 has 6 joints
...
def main(args):
    ...
    print("Meca500 Dataset 3D Visualization\n" + "=" * 50)
    ...
    import random
    num_samples = min(args.num_samples, len(json_files))
    selected_files = random.sample(json_files, num_samples)
    selected_samples = [os.path.join(JSON_DIR, f) for f in selected_files]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Visualize Meca500 robot pose predictions in 3D")
