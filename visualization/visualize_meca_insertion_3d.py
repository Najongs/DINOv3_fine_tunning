...
def visualize_3d_prediction(json_path, model, device='cuda'):
    """Visualize 3D model prediction on a single MecaInsertion sample."""
    ...
    pred_angles_np = pred_angles[0].cpu().numpy()[:6]  # MecaInsertion has 6 joints

    robot = MecaInsertionKinematics()
    joint_coords_3d_robot = robot.forward_kinematics(pred_angles_np)
    ...
    if len(selected_idx) < 4:
        ...
        if 'aruco_rvec_tvec' in sample['meta']:
            ...
            if 'rvec_x_deg' in sample['meta']['aruco_rvec_tvec']:
                rvec_deg = np.array([
                    sample['meta']['aruco_rvec_tvec']['rvec_x_deg'],
                    sample['meta']['aruco_rvec_tvec']['rvec_y_deg'],
                    sample['meta']['aruco_rvec_tvec']['rvec_z_deg']
                ], dtype=np.float32)
                rvec = np.deg2rad(rvec_deg)
            else:
                # Check if rvec is in degrees or radians
                if all(abs(v) > 2*np.pi for v in rvec_data):
                    # Likely in degrees, convert to radians
                    rvec = np.deg2rad(np.array(rvec_data, dtype=np.float32))
                else:
    ...
    # Get view and cam from metadata
    view = sample['meta'].get('view', 'unknown')
    cam = sample['meta'].get('cam', 'unknown')

    return gt_3d_camera, pred_3d_camera, summary, view, cam
...
def main(args):
    ...
    print("Meca Insertion Dataset 3D Visualization\n" + "=" * 50)
    ...
    for i, json_path in enumerate(selected_samples):
        ...
        try:
            gt_3d, pred_3d, joint_summary, view, cam = visualize_3d_prediction(json_path, model, device)
            plot_3d_comparison(ax, gt_3d, pred_3d,
                             f"View: {view.upper()} / Cam: {cam.upper()}\n{os.path.basename(json_path)}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Visualize Meca Insertion robot pose predictions in 3D")
